resource "vsphere_virtual_machine" "m-openst-controll" {
  name             = "m-openst-controller"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  host_system_id   = data.vsphere_host.esxi03.id
  datastore_id     = data.vsphere_datastore.esxi3-datastore1.id
  num_cpus         = 8
  memory           = 16384
  guest_id         = "ubuntu64Guest"
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  disk {
    label = "disk0"
    size  = 60
    unit_number = 0
  }
  clone {
    template_uuid = data.vsphere_virtual_machine.ubuntu-2204-template.id
    linked_clone = false
    customize {
      linux_options {
        host_name = "m-openst-controller"
        domain    = "ceph.local"
        time_zone = "Asia/Tehran"
        # script_text = 
      }
      network_interface {
        ipv4_address = "10.10.200.220"
        ipv4_netmask = 24
        # dns_domain   = "8.8.8.8"
      }
      #network_interface {}
      network_interface {
         ipv4_address = "192.168.200.220"
         ipv4_netmask = 24
       }
      ipv4_gateway = "10.10.200.1"
      dns_server_list = [ "8.8.8.8", "1.1.1.1" ]
    }
  }
  cdrom {
    client_device = true
  }
  vapp {
    properties = {
      "password"     = "123456789"
      "public-keys"  = "ssh-rsa  mkh@mkh-System-Product-Name"
      }
    }
  
  lifecycle {
    ignore_changes = [
      ept_rvi_mode,
      hv_mode
    ]
  }
}

resource "vsphere_virtual_machine" "m-openst-one" {
  name             = "openstack-one-m"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  host_system_id   = data.vsphere_host.esxi03.id
  datastore_id     = data.vsphere_datastore.esxi3-datastore1.id
  num_cpus         = 8
  memory           = 16384
  guest_id         = "ubuntu64Guest"
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  network_interface {
     network_id = data.vsphere_network.vm-network.id
  }
  disk {
    label = "disk0"
    size  = 40
    unit_number = 0
  }
  clone {
    template_uuid = data.vsphere_virtual_machine.ubuntu-2204-template.id
    linked_clone = false
    customize {
      linux_options {
        host_name = "openstack-one-m"
        domain    = "ceph.local"
        time_zone = "Asia/Tehran"
        # script_text = 
      }
      network_interface {
        ipv4_address = "10.10.200.221"
        ipv4_netmask = 24
        # dns_domain   = "8.8.8.8"
      }
      #network_interface {}
      network_interface {
        ipv4_address = "192.168.200.221"
        ipv4_netmask = 24
      }
      ipv4_gateway = "10.10.200.1"
      dns_server_list = [ "8.8.8.8", "1.1.1.1" ]
    }
  }
  cdrom {
    client_device = true
  }
  vapp {
    properties = {
      "password"     = "123456789"
      "public-keys"  = "ssh-rsa  mkh@mkh-System-Product-Name"
    
    }
  }
  lifecycle {
    ignore_changes = [
      ept_rvi_mode,
      hv_mode
    ]
  }
}

resource "vsphere_virtual_machine" "m-openst-two" {
  name             = "openstack-two-m"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  host_system_id   = data.vsphere_host.esxi03.id
  datastore_id     = data.vsphere_datastore.esxi3-datastore1.id
  num_cpus         = 8
  memory           = 16384
  guest_id         = "ubuntu64Guest"
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  disk {
    label = "disk0"
    size  = 40
    unit_number = 0
  }
  clone {
    template_uuid = data.vsphere_virtual_machine.ubuntu-2204-template.id
    linked_clone = false
    customize {
      linux_options {
        host_name = "openstack-two-m"
        domain    = "ceph.local"
        time_zone = "Asia/Tehran"
        # script_text = 
      }
      network_interface {
        ipv4_address = "10.10.200.222"
        ipv4_netmask = 24
        # dns_domain   = "8.8.8.8"
      }
      # network_interface {}
      network_interface {
        ipv4_address = "192.168.200.222"
        ipv4_netmask = 24
      }
      ipv4_gateway = "10.10.200.1"
      dns_server_list = [ "8.8.8.8", "1.1.1.1" ]
    }
  }
  cdrom {
    client_device = true
  }
  vapp {
    properties = {
      "password"     = "123456789"
      "public-keys"   = "ssh-rsa  mkh@mkh-System-Product-Name"
    }
  }
  lifecycle {
    ignore_changes = [
      ept_rvi_mode,
      hv_mode
    ]
  }
}

resource "vsphere_virtual_machine" "m-openst-three" {
  name             = "openstack-three-m"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  host_system_id   = data.vsphere_host.esxi03.id
  datastore_id     = data.vsphere_datastore.esxi3-datastore1.id
  num_cpus         = 8
  memory           = 16384
  guest_id         = "ubuntu64Guest"
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  disk {
    label = "disk0"
    size  = 40
  }
  clone {
    template_uuid = data.vsphere_virtual_machine.ubuntu-2204-template.id
    linked_clone = false
    customize {
      linux_options {
        host_name = "openstack-three-m"
        domain    = "ceph.local"
        time_zone = "Asia/Tehran"
        # script_text = 
      }
      network_interface {
        ipv4_address = "10.10.200.223"
        ipv4_netmask = 24
        # dns_domain   = "8.8.8.8"
      }
      # network_interface {}
      network_interface {
        ipv4_address = "192.168.200.223"
        ipv4_netmask = 24
      }
      ipv4_gateway = "10.10.200.1"
      dns_server_list = [ "8.8.8.8", "1.1.1.1" ]
    }
  }
  cdrom {
    client_device = true
  }
  vapp {
    properties = {
      "password"     = "123456789"
      "public-keys"   = "ssh-rsa  mkh@mkh-System-Product-Name"
    }
  }
  lifecycle {
    ignore_changes = [
      ept_rvi_mode,
      hv_mode
    ]
  }
}




resource "vsphere_virtual_machine" "m-openst-four" {
  name             = "openstack-four-m"
  resource_pool_id = data.vsphere_compute_cluster.cluster.resource_pool_id
  host_system_id   = data.vsphere_host.esxi03.id
  datastore_id     = data.vsphere_datastore.esxi3-datastore1.id
  num_cpus         = 8
  memory           = 16384
  guest_id         = "ubuntu64Guest"
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  network_interface {
    network_id = data.vsphere_network.vm-network.id
  }
  disk {
    label = "disk0"
    size  = 40
  }
  clone {
    template_uuid = data.vsphere_virtual_machine.ubuntu-2204-template.id
    linked_clone = false
    customize {
      linux_options {
        host_name = "openstack-four-m"
        domain    = "ceph.local"
        time_zone = "Asia/Tehran"
        # script_text = 
      }
      network_interface {
        ipv4_address = "10.10.200.224"
        ipv4_netmask = 24
        # dns_domain   = "8.8.8.8"
      }
      # network_interface {}
      network_interface {
        ipv4_address = "192.168.200.224"
        ipv4_netmask = 24
      }
      ipv4_gateway = "10.10.200.1"
      dns_server_list = [ "8.8.8.8", "1.1.1.1" ]
    }
  }
  cdrom {
    client_device = true
  }
  vapp {
    properties = {
      "password"     = "123456789"
      "public-keys"   = "ssh-rsa  mkh@mkh-System-Product-Name"
    }
  }
  lifecycle {
    ignore_changes = [
      ept_rvi_mode,
      hv_mode
    ]
  }
}